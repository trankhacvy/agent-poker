...
> {
  ...
  if (!accounts.bufferPda.value) {
    accounts.bufferPda.value = await getProgramDerivedAddress({
      programAddress:
      ...
      seeds: [
        ...
        getAddressEncoder().encode(expectAddress(accounts.pda.value)),
      ],
    });
  }
  if (!accounts.delegationRecordPda.value) {
    accounts.delegationRecordPda.value = await getProgramDerivedAddress({
      programAddress,
      seeds: [
  ...
  if (!accounts.delegationMetadataPda.value) {
    accounts.delegationMetadataPda.value = await getProgramDerivedAddress({
      programAddress,
      seeds: [
        getBytesEncoder().encode(
          new Uint8Array([
...
>(
  ...
  const getNextOptionalAccount = () => {
    ...
    return accountMeta.address === AGENT_POKER_GAME_PROGRAM_ADDRESS
      ? undefined
      : accountMeta;
  };
  return {
