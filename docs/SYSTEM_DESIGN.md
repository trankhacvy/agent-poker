...
| `TableVault` | `[b"table_vault", table_id_bytes]` | SystemAccount collecting all wagers |
...
TableEscrow
├── table_id: u64
├── wager_tier: u64            // lamports per seat
├── players: [Pubkey; 6]       // agent pubkeys (default = empty)
├── player_count: u8
...
| Instruction | Access Control | Description |
|-------------|----------------|-------------|
| `initialize_treasury()` | Authority | Creates singleton treasury PDA (idempotent) |
| `create_table(table_id, wager_tier)` | Authority | Creates table with status=Open |
| `join_table()` | Agent owner | Transfers wager from agent vault to table vault. Auto-transitions to Full at 6 players. |
| `start_game()` | Authority (requires Full) | Marks table InProgress |
| `settle_table(winner_pubkey)` | Authority | Distributes 95% to winner vault, 5% to treasury |
| `refund_table()` | Authority | Returns wager to each player's vault (Open/Full only) |
...
| `PlayerHand` | `[b"player_hand", game_id_bytes, seat_index]` | 2 hole cards per player — PERMISSIONED |
...
|-------------|-------------|
| `create_game(game_id, table_id, wager_tier)` | Creates GameState + 6 PlayerHand accounts, phase=Waiting |
| `join_game(game_id, seat_index, player_pubkey)` | Register player at seat, create MagicBlock permission, delegate hand PDA to ER |
| `start_game(game_id)` | Delegate GameState to ER via MagicBlock delegation system |
| `request_shuffle(game_id, client_seed)` | CPI to MagicBlock VRF program for randomness |
| `callback_shuffle(randomness)` | VRF callback: Fisher-Yates shuffle, deal cards, post blinds, phase=Preflop |
| `player_action(action, raise_amount)` | Fold/Check/Call/Raise/AllIn. Validates turn, updates pot/bets, advances phase. |
| `showdown()` | Evaluate all remaining hands (7-card best-5), determine winner, emit GameFinished event |
...
```
Agent ↔ Escrow:
  Escrow validates agent vault PDAs via seeds::program constraint.
  Transfers to/from agent vaults during join/settle/refund.

Game → MagicBlock:
  Uses #[delegate], #[commit], #[vrf] macros for delegation, commitment, and VRF.
  CPI to MagicBlock permission and VRF programs.
...
5. PER permissions: GameState readable by game server authority; PlayerHand readable by game authority (for LLM input)
6. Game plays out inside PER (~50ms per transaction)
...
async runGame(config: GameConfig):
    ...
    // 12. Return result, broadcast game_end
